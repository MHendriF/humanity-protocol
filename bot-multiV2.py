...
WALLETS_FILE = 'wallet.txt'  # File untuk menyimpan private keys
PROXIES_FILE = 'proxy.txt'   # File untuk menyimpan proxy
FAUCET_API_URL = "https://faucet.testnet.humanity.org/api/claim"
CONTRACT_ADDRESS = '0x'      # Ganti dengan alamat kontrak yang sesuai
CONTRACT_ABI = [...]         # ABI kontrak tetap sama

# Fungsi untuk membaca file wallet.txt
def load_wallets():
    try:
        ...
        console.print(f"{Fore.RED}🚨 File {WALLETS_FILE} tidak ditemukan!")
        sys.exit(1)
...
def load_proxies():
    try:
        with open(PROXIES_FILE, 'r') as file:
            ...
            console.print(f"[bold blue]🌐 {len(proxies)} Proxy ditemukan![/bold blue]")
            return proxies
    except FileNotFoundError:
        console.print(f"{Fore.RED}🚨 File {PROXIES_FILE} tidak ditemukan!")
        sys.exit(1)

# Fungsi untuk menghubungkan ke blockchain
...
# Fungsi untuk klaim faucet dengan proxy
def claim_faucet(wallet, proxy):
    ...
    while True:
        ...
        try:
            response = requests.post(FAUCET_API_URL, json=payload, headers=headers, proxies=proxies)
...
def claim_reward(wallets, proxies, web3, contract):
    for i, wallet in enumerate(wallets):
        proxy = proxies[i % len(proxies)]  # Round-robin untuk memilih proxy
        try:
            ...
            if genesis_claimed and not claim_status:
                ...
                process_claim(sender_address, wallet["private_key"], web3, contract, proxy)
...
# Fungsi untuk memproses klaim reward dengan proxy
def process_claim(sender_address, private_key, web3, contract, proxy):
    try:
        max_retries = 15
        retry_count = 0
